...
type SourceSchema []Attribute

func (d SourceSchema) Parse(fields []string) (r *Record) {
	rid, err := strconv.ParseUint(fields[0], 0, 32)
	if err != nil {
		panic(err)
	}
	r = &Record{uint32(rid), make(map[string]string)}
	for _, a := range d {
		r.Fields[a.ShortName] = fields[a.Column - 1]
	}
